...
def w_variable_variance(shape):
  initial = tf.random_normal(shape = shape, stddev = 0.001, seed = seed) - 5.0 # mean 0 stddev 1
  return tf.Variable(initial)
...
def create_generator(n_units_gen, noise_comps_gen, total_number_z, n_layers_gen):
    ...
    log_var_noise = w_variable_mean([ 1, noise_comps_gen ])
...
def compute_outputs_main_NN(z_samples, dict_main_NN, dict_dim_weights, x_input, y_target, n_samples, training = 0):
    ...
    A3 = tf.transpose(A3, [ 1, 0, 2]) # A3 should be n_batch x n_samples x 10 instead of n_samples x n_batch x 10
...
def main( ):
    ...
    mean_weights , var_weights = tf.nn.moments(weights, axes = [0]) # We use empirical estimators
    ...
    # KL and res_train have shape batch_size x n_samples

    mean_KL = tf.reduce_mean(KL)

    # Create the variable lists to be updated

    vars_primal = get_variables_generator(generator) + get_variables_main_NN(main_CNN) + [ log_vars_prior ]
    ...
    with tf.Session(config = tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch in range(n_batches_train):
                ...
                if ((i_batch + 1) % batches_to_report) == 0:
                    string = ('\n AVB - batch %g datetime %s epoch %d ELBO %g CROSS-ENT %g KL %g annealing_factor %g \
        ...
        error_class_se = np.std(cl_errors_cont) / len(cl_errors_cont)
        TestLL_se = np.std(LL_cont) / len(LL_cont)
        accuracy_test_se = np.std(acc_cont) / len(acc_cont)
        brier_test_se = np.std(brier_cont) / len(brier_cont)
        ...
        string = ('AVB - batch %g datetime %s epoch %d ELBO %g CROSS-ENT %g KL %g real_time %g cpu_time %g ' + \
            'test_time %g total_time %g KL_factor %g ; LL %g - LL_error %g; Classification_error %g - Class_error %g ; ' + \
            'Accuracy %g - Acc_error %g ; Brier-score %g Brier_error %g') % \
            ...
            kl_factor, TestLL, TestLL_se, error_class, error_class_se, accuracy_test, accuracy_test_se, brier_test, brier_test_se)
        print(string)
        sys.stdout.flush()
        ...
        np.savetxt('res_avb/results_error.txt', [ error_class ])
        np.savetxt('res_avb/results_ll.txt', [ TestLL ])
        np.savetxt('res_avb/results_accuracy.txt', [ accuracy_test ])
        np.savetxt('res_avb/results_brier.txt', [ brier_test ])

        np.savetxt('res_avb/results_error_se.txt', [ error_class_se ])
        np.savetxt('res_avb/results_ll_se.txt', [ TestLL_se ])
        np.savetxt('res_avb/results_accuracy_se.txt', [ accuracy_test_se ])
        np.savetxt('res_avb/results_brier_se.txt', [ brier_test_se ])

        np.savetxt('res_avb/forecasts.txt', final_forecasts)
        np.savetxt('res_avb/labels.txt', final_labels)
