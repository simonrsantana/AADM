...
def compute_outputs_main_NN(z_samples, dict_main_NN, dict_dim_weights, x_input, y_target, alpha, n_samples, training = 0):
    ...
    res_train = 1.0 / alpha * (- tf.log(tf.cast(n_samples, tf.float32)) + tf.reduce_logsumexp( alpha * \
        -1.0 * tf.nn.sparse_softmax_cross_entropy_with_logits(labels = labels, logits = A3), axis = 1))
...
def main(alpha):
    ...
    T_real = compute_output_discriminator(discriminator, weights, n_layers_disc)
    ...
    res_train, log_prob_data, classification_error, alt_accuracy, forecasts, real_labels, multiclass_brier = \
        compute_outputs_main_NN(z_samples, main_CNN, dict_dim_weights, x, y_, alpha, n_samples, training)
    ...
    with tf.Session(config = tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for i_batch in range(n_batches_train):
                ...
                if ((i_batch + 1) % batches_to_report) == 0:
                    string = ('\n alpha %g batch %g datetime %s epoch %d ELBO %g CROSS-ENT %g KL %g annealing_factor %g \
                        ...
                        (alpha, i_batch + 1, str(datetime.now()), epoch + 1, \
        ...
        # error_class = cl_errors / n_batches_to_process
        # TestLL = (LL / n_batches_to_process)
        # accuracy_test = (acc / n_batches_to_process)
        # brier_test = (brier / n_batches_to_process)
        ...
        np.savetxt('res_alpha/' + str(alpha) + 'results_brier.txt', [ brier_test ])

        np.savetxt('res_alpha/' + str(alpha) + 'results_error_var.txt', [ error_class_var ])
        np.savetxt('res_alpha/' + str(alpha) + 'results_ll_var.txt', [ TestLL_var ])
        np.savetxt('res_alpha/' + str(alpha) + 'results_accuracy_var.txt', [ accuracy_test_var ])
        np.savetxt('res_alpha/' + str(alpha) + 'results_brier_var.txt', [ brier_test_var ])
