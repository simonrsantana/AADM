...
def main(permutation, split, layers):
    ...
    if layers == 1:
        res_train, vars_network, squared_error, log_prob_data_test, res_test = exp_log_likelihood(mean_targets, std_targets, weights_train, weights_test) # main loss in the VAE
    elif layers == 2:
        res_train, vars_network, squared_error, log_prob_data_test, res_test, y_test_pre, A3_test = exp_log_likelihood_double(mean_targets, std_targets, weights_train, weights_test) # main loss in the VAE
    ...
    with tf.Session(config = config) as sess:
        ...
        for epoch in range(n_epochs):
            ...
            with open("prints/prints_AVB_boston/results_avb_" + str(split) + "_" +  original_file, "a") as res_file:
        ...
        np.savetxt('results_AVB_boston/results_rmse_' + str(split) + '.txt', [ RMSE ])
        np.savetxt('results_AVB_boston/results_ll_' + str(split) + '.txt', [ TestLL ])
        np.savetxt('results_AVB_boston/results_time_' + str(split) + '.txt', [ timing ])
        np.savetxt('results_AVB_boston/results_KL_' + str(split) + '.txt', [ tmp_kl ])
        np.savetxt('results_AVB_boston/results_ELBO_' + str(split) + '.txt', [ tmp_elbo ])
...
if __name__ == '__main__':
    ...
    if not os.path.isdir("results_AVB_boston"):
        os.makedirs("results_AVB_boston")

    # Create a folder to store the screen prints
    if not os.path.isdir("prints"):
        os.makedirs("prints")

    if not os.path.isdir("prints/prints_AVB_boston"):
        os.makedirs("prints/prints_AVB_boston")

    # Create (or empty) the results of the run
    if os.path.isfile("prints/prints_AVB_boston/results_avb_" + str(split) + "_" +  original_file):
        with open("prints/prints_AVB_boston/results_" + str(split) + "_" +  original_file, "w") as res_file:
           res_file.close()
